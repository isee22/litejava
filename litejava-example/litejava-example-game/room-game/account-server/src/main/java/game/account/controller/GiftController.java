...
public class GiftController {
    ...
    public static void register(App app) {
        ...
        app.post("/gift/send", ctx -> {
            ...
            Services.gift.sendGift(req.fromUserId, req.toUserId, req.giftId, req.count);
            ctx.ok(null);
        });
        ...
        app.get("/gift/charm/:userId", ctx -> {
            ...
            int charm = Services.gift.getCharm(userId);
        ...
        app.get("/gift/received/:userId", ctx -> {
            long userId = Long.parseLong(ctx.pathParam("userId"));
            List<GiftService.GiftRecord> gifts = Services.gift.getReceivedGifts(userId, 50);
            ctx.ok(gifts);
        });
        ...
        app.get("/gift/sent/:userId", ctx -> {
            long userId = Long.parseLong(ctx.pathParam("userId"));
            List<GiftService.GiftRecord> gifts = Services.gift.getSentGifts(userId, 50);
            ctx.ok(gifts);
        });
    }
}
